<?php
	...
	class UploadComponent extends Component {
		...
		public function upload ($file, $destination, $name = NULL, $rules = NULL, $allowed = NULL) {
			...
			if (!is_null($allowed)) { $this->_allowed = $allowed; } else { $this->_allowed = array('jpg','jpeg','gif','png','JPG','JPEG'); }
			...
			if (isset($file) && is_array($file) && !$this->upload_error($file['error'])) {
			    ...
			    // -- error if name of file is grather than 50 
               if(strlen(trim(str_replace($destination,'',$fileName))) > 500) {
                   $this->error("File name can not be more than 50 characters long.");
                   return;
               }
