...
Ext.onReady(function(){	
	var myData = [
		...
		$name= ucwords($value['tmp_table']['fName'])." ".ucwords($value['tmp_table']['lName'])." (".$value['tmp_table']['rollNumber'].")";
		if($value[0]['percentageScore']>=$proficientPercentage){ 
			$image = $this->Html->image('text_tick.gif'); 
		}else{ 
			$image = $this->Html->image('text_cross.gif'); 
		}
		if($value[0]['percentageScore']>=0){
			$percentageScore = number_format($value[0]['percentageScore'],0);
		}else{
			$percentageScore = 0;
		}
		 $tmpArr[]=<<<STR
		["{$name}",'{$image}',"{$value['tmp_table']['score']}","{$percentageScore}","{$value[0]['correctAnswer']}","{$value['tmp_table']['noOfIncorrectQ']}","{$value['tmp_table']['noOfSkippedQ']}","{$value['tmp_table']['rank']}","{$this->Util->getGrade($value[0]['percentageScore'])}"]
STR;
		}
		echo implode(",",$tmpArr);
		}
    ...
    function pctChange(val){
        ...
        if(val >= <?php echo $proficientPercentage;?>){
...
foreach($outcomeResult as $value){
	$tmpArr[$value['StudentDetails']['userId']][$value['0']['sloId']]=$value;
	$tmpArr[$value['StudentDetails']['userId']]['userName']=ucwords($value['StudentDetails']['fName'])." ".ucwords($value['StudentDetails']['lName'])." (".$value['StudentDetails']['rollNumber'].")";
}
...
foreach($sloResult as $sloDetails){	
	$totalMarks=isset($outcomeDetails[$sloDetails['0']['sloId']][0]['totalMarks'])?$outcomeDetails[$sloDetails['0']['sloId']][0]['totalMarks']:1;
	$totalMarksObtain=isset($outcomeDetails[$sloDetails['0']['sloId']][0]['totalMarksObtain'])?$outcomeDetails[$sloDetails['0']['sloId']][0]['totalMarksObtain']:0;
...
foreach($sloResult as $key=>$value){
	$qRow .= ",{header:'Outcome".$sloArr[$value['0']['sloId']]['LessonNo']."',width: 90, renderer: showFlag,  sortable: true, dataIndex: 'Outcome".$key."'}";
...
<?php foreach($sloResult as $key=>$value){
	echo '<li style="line-height:20px;font-size:12px;">Outcome '.$sloArr[$value['0']['sloId']]['LessonNo'].' : '.$value['0']['title']. '</li>';
} ?>
</ul>
</div>
</div>